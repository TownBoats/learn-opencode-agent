...
- 技术栈：Anthropic SDK + TypeScript（不依赖 OpenCode 框架）
...
2. **标题**：「AI Agent 实战手册」+ 副标题「23 个项目 · 每章可运行 · Anthropic SDK + TypeScript」
...
**伪代码逻辑**：
```
lines = ["$ bun run p01-minimal-agent.ts", "✓ Anthropic SDK initialized", ...]
...
<ProjectCard
  ...
  duration="45 min"
  :prerequisites="['P01']"
  :tags="['Tool Calling', 'Anthropic SDK']"
/>
