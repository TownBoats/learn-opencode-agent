...
- 示例文件位置：仓库根目录 `p14-mcp.ts`、`p14-mcp-server.ts`
...
async function connectMcpServer(client: Client): Promise<OpenAI.ChatCompletionTool[]> {
  ...
  return tools.map((t) => ({
    ...
    input_schema: t.inputSchema as OpenAI.ChatCompletionTool['input_schema'],
  }))
}
```
...
// Agent 主循环
async function runAgent(client: Client, tools: OpenAI.ChatCompletionTool[], userMessage: string): Promise<void> {
  ...
  while (true) {
    ...
    const textBlocks = response.content.filter(
      (b): b is OpenAI.ChatCompletionMessage => b.type === 'text'
    )
...
| `inputSchema` -> `input_schema` | MCP 工具描述用 camelCase `inputSchema`，OpenAI SDK 用 snake_case `input_schema`，转换时只需重命名这一个字段 |
