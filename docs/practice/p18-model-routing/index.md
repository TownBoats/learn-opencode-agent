...
- 示例文件位置：仓库根目录 `p18-model-routing.ts`
...
硬编码 `model: 'claude-sonnet-4-20250514'` 这类具体型号在原型阶段没问题，但一旦上线你会发现：
...
Anthropic 的公开定价天然支持这种分层策略——不同模型之间存在明显价格差，用对模型会直接影响 API 成本。
...
按这张示例表计算，Haiku 到 Opus 的输入价格差距约为 **19 倍**。这也是为什么模型路由通常能带来明显的成本收益。
...
const MODELS: Record<ModelTier, ModelConfig> = {
  haiku: {
    id: 'claude-haiku-4-5-20251001', // 示例型号，请以官方模型页为准
    tier: 'haiku',
  ...
  sonnet: {
    id: 'claude-sonnet-4-20250514', // 示例型号，请以官方模型页为准
    tier: 'sonnet',
  ...
  opus: {
    id: 'claude-opus-4-6', // 示例型号，请以官方模型页为准
    tier: 'opus',
...
| 三层模型策略 | Haiku 处理简单任务，Sonnet 处理通用任务，Opus 处理复杂推理；具体型号与价格请以官方模型页、定价页为准 |
