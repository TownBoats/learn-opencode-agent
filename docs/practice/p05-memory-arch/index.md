...
<ProjectCard
  ...
  duration="45 min"
  :prerequisites="['P1', 'P2']"
  :tags="['Memory', 'State Management', 'TypeScript', 'Anthropic SDK']"
/>
...
- 建议先完成前置章节：`P1`、`P2`
- 本章建议入口命令：`bun run p05-memory-arch.ts`
- 示例文件位置：仓库根目录 `p05-memory-arch.ts`
...
<RunCommand command="bun run p05-memory-arch.ts" :verified="true" />
...
| 长期记忆 | 持久化 JSON，通过 `system` prompt 注入，实现跨会话"记忆" |
