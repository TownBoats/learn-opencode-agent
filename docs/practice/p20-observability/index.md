...
- 示例文件位置：仓库根目录 `p20-observability.ts`
...
(+) agent_turn 3210ms [user_message=帮我搜索关于 Agent 可观测性的文档..., iterations=2]
  ├── (+) llm_call 1842ms [model=gpt-4o, iteration=1, input_tokens=312, output_tokens=156, stop_reason=tool_use]
  ...
  ├── (+) llm_call 1203ms [model=gpt-4o, iteration=2, input_tokens=580, output_tokens=210, stop_reason=end_turn]
══════════════════════════════════════════
