...
- 示例文件位置：仓库根目录 `p13-multimodal.ts`
...
// 将 ImageSource 转换为 OpenAI SDK 需要的 ImageBlockParam
function toImageBlock(source: ImageSource): OpenAI.ChatCompletionContentPartImage {
  if (source.type === 'url') {
    return {
      type: 'image',
      source: {
...
class MultimodalAgent {
  ...
  // 这里使用示例型号；实际项目中请替换为你账户当前可用的视觉模型 ID
  ...
  async compareImages(images: ImageSource[], question: string): Promise<string> {
    ...
    const content: OpenAI.ChatCompletionContentPartParam[] = [
...
// 注意：OpenAI API 不支持 SVG，需要使用 PNG/JPEG。
...
| 概念 | 说明 |
|------|------|
| `ImageBlockParam` | OpenAI SDK 的图像 block 类型，source 字段支持 `base64` 和 `url` 两种来源 |
...
- `toImageBlock` 负责将 `ImageSource` 转换为 OpenAI SDK 的 `ImageBlockParam`
